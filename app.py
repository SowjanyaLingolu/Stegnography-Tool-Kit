import os
import shutil
from stegano import lsb #stegno
from PIL import Image
from pydub import AudioSegment #pydub
from flask import Flask, request, Response, render_template, send_file, redirect, url_for, session #flask
...
def encode_image(file_path, message):
     try:
        ...
        orig_format = myimg.format  # Get original format (PNG, JPG, etc.)

        # Ensure lossless encoding (JPG is problematic)
        if orig_format == "JPEG":
            myimg = myimg.convert("RGB")
            new_file_path = file_path.rsplit('.', 1)[0] + ".png"  # BMP avoids compression
        else:
            new_file_path = file_path.rsplit('.', 1)[0] + "." + orig_format.lower()

        newimg = myimg.copy()
        encode_enc(newimg, message)

        # Save in original format (or BMP for JPG)
        newimg.save(new_file_path, format="BMP" if orig_format == "JPEG" else orig_format)
...
def decode_image(file_path):
    try:
        # Open and convert the encoded image to RGBA format for decoding
...
# Text Steganography
def encode_text(input_path, secret_message):
    ...
    for i in range(len(words)):
        ...
        if binary_index < len(binary_message):
            # If the bit is '1', we add a double space after the word
            if binary_message[binary_index] == '1':
                encoded_content.append(word + '  ')  # Add two spaces
            else:
                encoded_content.append(word + ' ')  # Add one space
            
            binary_index += 1
        else:
            # If no more bits to encode, just add the word without changing it
            encoded_content.append(word)
    
    # Join the words back together with a single space in between
    encoded_text = ' '.join(encoded_content)

    # Overwrite the original file with the encoded content
    with open(input_path, 'w', encoding='utf-8') as f:
        f.write(encoded_text)

    print(f"Message encoded and saved to {input_path}")
    return input_path

def decode_text(input_path):
    # Open the encoded file and read its content
    with open(input_path, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    binary_message = ""
    for word in words:
        # Trim the word to ensure no trailing spaces are present
        word = word.rstrip()  # Strip trailing spaces

        # If the word ends with a double space, it's a '1', otherwise it's a '0'
        if word.endswith('  '):
            binary_message += '1'
        elif word.endswith(' '):
            binary_message += '0'

    # Split the binary message into chunks of 8 bits (1 byte) to decode each character
    decoded_message = ""
    for i in range(0, len(binary_message), 8):
        byte = binary_message[i:i+8]
        if len(byte) == 8:
